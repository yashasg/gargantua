...
/// that cross-client gap. Caches are created lazily on first use and live for
/// the process lifetime — connection counts are small (one stdio + a handful
/// of SSE sessions) so no eviction is needed.
